...
const EmployeeDashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [statsResponse, complaintsResponse, tasksResponse, transactionsResponse] = await Promise.all([
        axiosInstance.get('http://147.93.53.119:5000/employee/dashboard/stats', { headers }),
        axiosInstance.get('http://147.93.53.119:5000/employee/dashboard/recent_complaints', { headers }),
        axiosInstance.get('http://147.93.53.119:5000/employee/dashboard/pending_tasks', { headers }),
        axiosInstance.get('http://147.93.53.119:5000/employee/dashboard/recent_inventory_transactions', { headers })
      ]);
  ...
  const handleCompleteTask = async (taskId: string) => {
    try {
      const token = getToken();
      await axiosInstance.patch(`/api/tasks/${taskId}`, { status: 'completed' }, {
  ...
  return (
    <div className="flex h-screen bg-[#F1F0E8]">
      ...
      <div className={`flex flex-col flex-1 overflow-hidden transition-all duration-300 ease-in-out`}>
        ...
        <main className={`flex-1 overflow-x-hidden overflow-y-auto bg-[#F1F0E8] p-6 pt-20 ${isSidebarOpen ? 'ml-72' : 'ml-20'}`}>
          <h1 className="text-3xl font-semibold text-[#89A8B2] mb-6">Employee Dashboard</h1>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
            <StatCard title="Open Complaints" value={stats?.open_complaints.toString() || '0'} icon={AlertCircle} color="#B3C8CF" />
            <StatCard title="Pending Tasks" value={stats?.pending_tasks.toString() || '0'} icon={CheckSquare} color="#E5E1DA" />
            <StatCard title="Assigned Inventory" value={stats?.assigned_inventory.toString() || '0'} icon={Package} color="#89A8B2" />
            <StatCard title="Inventory Transactions" value={stats?.inventory_transactions.toString() || '0'} icon={BarChart2} color="#B3C8CF" />
          </div>
      ...
      <ul className="space-y-4">
        {complaints.map((complaint) => (
          <li key={complaint.id} className="flex justify-between items-center border-b pb-2">
            <div>
              <p className="font-medium text-[#B3C8CF]">{complaint.title}</p>
      ...
      <ul className="space-y-4">
        {transactions.map((transaction) => (
          <li key={transaction.id} className="flex justify-between items-center border-b pb-2">
            ...
            <span className="text-xs text-[#89A8B2]">
