  ...
  useEffect(() => {
    ...
    const fetchCustomerData = async () => {
      try {
        ...
        const customerResponse = await axiosInstance.get(`http://mbanet.com.pk/api/customers/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        setCustomer(customerResponse.data)

        const invoicesResponse = await axiosInstance.get(`http://mbanet.com.pk/api/invoices/customer/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        setInvoices(invoicesResponse.data)

        const paymentsResponse = await axiosInstance.get(`http://mbanet.com.pk/api/payments/customer/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        setPayments(paymentsResponse.data)

        const complaintsResponse = await axiosInstance.get(`http://mbanet.com.pk/api/complaints/customer/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
  ...
  useEffect(() => {
    const fetchCnicImage = async () => {
      if (customer && customer.cnic_front_image && customer.cnic_back_image) {
        try {
          const token = getToken()
          const responseFront = await axiosInstance.get(`http://mbanet.com.pk/api/customers/cnic-front-image/${id}`, {
            headers: { Authorization: `Bearer ${token}` },
            responseType: "blob",
          })
          const responseBack = await axiosInstance.get(`http://mbanet.com.pk/api/customers/cnic-back-image/${id}`, {
            headers: { Authorization: `Bearer ${token}` },
            responseType: "blob",
          })
                      ...
                      <div>
                        ...
                        {customer.agreement_document ? (
                          <button
                            onClick={() => {
                              const token = getToken()
                              fetch(`http://mbanet.com.pk/api/customers/agreement-document/${customer.id}`, {
                                headers: { Authorization: `Bearer ${token}` },
                              })
