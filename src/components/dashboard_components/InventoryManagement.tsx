...
export const InventoryManagement: React.FC = () => {
  ...
  useEffect(() => {
    const fetchInventoryData = async () => {
      try {
        const response = await axiosInstance.get('http://147.93.53.119:5000/dashboard/inventory-management', {
          headers: {
  ...
  return (
    <div className="grid grid-cols-2 gap-4 mb-6">
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={inventoryData.stock_level_data}>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <h2 className="text-xl font-semibold mb-4">Inventory Transaction Trends</h2>
        <div className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={inventoryData.inventory_transaction_data}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="month" />
              <YAxis />
              <Tooltip />
      ...
      <div className="bg-white rounded-lg shadow-md p-6 md:col-span-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-blue-100 p-4 rounded-lg">
            ...
            <p className="text-2xl font-bold text-blue-900">${inventoryData.inventory_metrics.total_inventory_value.toLocaleString()}</p>
          </div>
          <div className="bg-green-100 p-4 rounded-lg">
            <h4 className="text-sm font-medium text-green-800">Inventory Turnover Rate</h4>
            <p className="text-2xl font-bold text-green-900">{inventoryData.inventory_metrics.inventory_turnover_rate}</p>
          </div>
          <div className="bg-yellow-100 p-4 rounded-lg">
            <h4 className="text-sm font-medium text-yellow-800">Low Stock Items</h4>
            <p className="text-2xl font-bold text-yellow-900">{inventoryData.inventory_metrics.low_stock_items}</p>
          </div>
          <div className="bg-purple-100 p-4 rounded-lg">
            <h4 className="text-sm font-medium text-purple-800">Average Assignment Duration</h4>
            <p className="text-2xl font-bold text-purple-900">{inventoryData.inventory_metrics.avg_assignment_duration} days</p>
          </div>
        </div>
      </div>
    </div>
