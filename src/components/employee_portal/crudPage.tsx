...
export function CRUDPage<T extends { id: string; is_active?: boolean }>({ 
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get(`http://mbanet.com.pk/api/${endpoint}/list`, {
  ...
  const handleToggleStatus = async (id: string, currentStatus: boolean) => {
    try {
      const token = getToken();
      await axiosInstance.put(`http://mbanet.com.pk/api/${endpoint}/update/${id}`, 
  ...
  const handleBulkStatusChange = async (newStatus: boolean) => {
    try {
      ...
      await Promise.all(selectedRows.map(id => 
        axiosInstance.put(`http://mbanet.com.pk/api/${endpoint}/update/${id}`, 
          { is_active: newStatus },
          { headers: { Authorization: `Bearer ${token}` } }
        )
      ));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let response;
      if (editingItem) {
        response = await axiosInstance.put(`http://mbanet.com.pk/api/${endpoint}/update/${editingItem.id}`, formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        response = await axiosInstance.post(`http://mbanet.com.pk/api/${endpoint}/add`, formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
