...
export function CRUDPage<T extends { id: string; is_active?: boolean }>({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = getToken();
      if (editingItem) {
        await axiosInstance.put(`http://mbanet.com.pk/api/${endpoint}/update/${editingItem.id}`, formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
        toast.success(`${title} updated successfully`);
      } else {
        await axiosInstance.post(`http://mbanet.com.pk/api/${endpoint}/add`, formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
