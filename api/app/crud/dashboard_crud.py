...
def get_executive_summary_data(company_id):
    try:
        ...
        print(f"Error fetching executive summary data: {e}")
        return {
...
def get_customer_analytics_data(company_id):
    try:
        ...
        print(f"Error fetching customer analytics data: {e}")
        return {
...
def get_financial_analytics_data(company_id):
    try:
        ...
        print(f"Error fetching financial analytics data: {e}")
        return {
...
def get_stock_level_data(company_id):
    try:
        ...
        return [{'name': name, 'quantity': int(quantity)} for name, quantity in stock_levels]
    except Exception as e:
...
def get_inventory_metrics(company_id):
    try:
        # Note: Removed InventoryTransaction.unit_price as it's not present in the current model
        total_value = db.session.query(func.sum(InventoryItem.quantity)
        ).join(Supplier
        ).filter(Supplier.company_id == company_id).scalar() or 0
        
        annual_assignments = db.session.query(func.count(InventoryTransaction.id)
        ).join(InventoryItem
        ).join(Supplier
        ).filter(Supplier.company_id == company_id,
                 InventoryTransaction.transaction_type == 'assignment',
                 InventoryTransaction.performed_at >= datetime.utcnow() - timedelta(days=365)).scalar() or 0
        ...
        avg_assignment_duration = db.session.query(func.avg(
            InventoryAssignment.returned_at - InventoryAssignment.assigned_at
        )).join(InventoryItem
        ).join(Supplier
        ).filter(Supplier.company_id == company_id,
                 InventoryAssignment.returned_at.isnot(None)).scalar()

        avg_assignment_duration = round(avg_assignment_duration.days) if avg_assignment_duration else 0
